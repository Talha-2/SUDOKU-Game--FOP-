...
{
    for (int i = 0; i < GRID_SIZE; i++)  //////////to check row and column
    {
    ...
    for (int i = 0; i < 3; i++)    ////////////// to check 3*3 grid
    {
        for (int j = 0; j < 3; j++) {
...
void playGame(sf::RenderWindow& window, int difficulty) {
    ...
    if (!font.loadFromFile("digitalism.ttf")) {
        std::cerr << "Error loading font file." << endl;
        return;
    }
...
int main()
{
    ...
    int difficulty=2; ////////default medium level
